...
void replay_production_skim_NPS_HMS(int RunNumber=0, int MaxEvent=0, int FirstEvent = 1, int MaxSegment = 1, int FirstSegment = 0, const char* fname_prefix = "nps_coin")
{
  ...
  // Define DEF-file+
  analyzer->SetOdefFile("DEF-files/NPS/NPS_skim.def"); //FIXME: CHANGE
  // Define cuts file
  analyzer->SetCutFile("DEF-files/NPS/NPS_cuts_coin.def"); //FIXME: CHANGE
  // File to record accounting information for cuts
  //analyzer->SetSummaryFile(Form("REPORT_OUTPUT/COIN/SKIM/summary_production_%d_%d.report", RunNumber, MaxEvent));  // optional //FIXME: CHANGE
